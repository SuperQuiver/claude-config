...
- push 前 → SESSION.md/CLAUDE.md が実態と一致か確認（詳細は CONVENTIONS.md §3）
- 詳細は `~/Claude/CONVENTIONS.md` 参照
```
...
- **画像出力**: ファイル名は内容を反映（バージョン番号不可）。生成後は `open` で表示
- **GFM ルール**: 日本語 bold 対策の詳細は `~/Claude/claude-config/gfm-rules.md` 参照
- **odakin のリポのみ操作**。yohey-w は絶対に触らない
